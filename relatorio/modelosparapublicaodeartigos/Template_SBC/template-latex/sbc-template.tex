...
A aplicação foi desenvolvida em PHP como linguagem server-side, com auxilio de css e html para client-side. A aplicação web desenvolvida para consulta dos dados conta com uma divisão entre algumas entidades do banco e também alguns atributos. São elas Escolas, Estados e Regiões. Foram também oferecidas outras formas de consulta como um campo para consultas escritas diretamente em sql.Para cada divisão são fornecidas oportunidades de consulta baseadas na divisão escolhida.

Na aba Escolas temos disponível a listagem de todas as escolas disponíveis como página inicial da divisão. Fica disponível para consultar as taxas e telefones de uma escola por nome dentre outros atributos.
Na aba de Estados, é disponibilizada uma listagem com todas as escolas por Estado como página inicial da divisão. Fica disponível também a média de taxas de aprovação do terceiro ano do ensino medio por Estado.
Na aba Regiões fica disponível a listagem de Escolas por região, como também a contagem de escolas com taxas de aprovação acima de certo valor.
